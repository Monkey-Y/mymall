...
public class RedisPool {
    //用static的原因：我们要保证jedis连接池在我们Tomcat启动的时候要加载出来，我们后面会写一个静态代码块初始化他
    ...
    private static Boolean testOnBorrow = Boolean.parseBoolean(PropertiesUtil.getProperty("redis.test.borrow","true"));//程序在从jedispool中borrow一个jedis实例的时候，是否要进行验证操作，如果赋值true。则得到的jedis实例肯定是可以用的。
    ...
    //这个类在加载到JVM时就要初始化这个连接池
    static{
        initPool();
    }

    //把redis连接池里面的jedis实例开放出去
    ...
    public static void main(String[] args) {
        ...
        pool.destroy();//临时调用，销毁连接池中的所有连接。在业务中会重复使用，不要销毁。
